...
// Find finds the node with the given data or the node closest above the passed argument.
...
// Returns true if there was a change (new node inserted or existing node updated), false otherwise.
...
func (t *AATree[X]) find(node *treeNode[X], data X) (x X, out bool) {
	for node != nil {
		c := t.compare(data, node.data)
		if c > 0 {
			node = node.right
		...
		if c != 0 {
			...
			within, ok := t.find(node.left, data)
			if ok {
				return within, true
			}
		}
...
func (t *AATree[X]) insert(node *treeNode[X], data X) *treeNode[X] {
	...
	if c < 0 {
		...
		node.data = data
		t.change = true // always set change=true here; not comparable
